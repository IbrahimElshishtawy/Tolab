...
macro(_FPHSA_FAILURE_MESSAGE _msg)
  ...
  string(APPEND __msg "\n"
    "You may need to set the environment variables ${_NAME}_DIR or ${_NAME}_ROOT to point to the root of the installation of ${_NAME}.\n"
    "If you have installed ${_NAME} in a non-standard location, you may want to use the CMake variable CMAKE_PREFIX_PATH to specify the search path.\n")
  endif()
  if (FPHSA_FAIL_MESSAGE STREQUAL "DEFAULT_MSG")
    message(FATAL_ERROR "${__msg}")
    else()
    if (FPHSA_FAIL_MESSAGE)
      string(PREPEND __msg "${FPHSA_FAIL_MESSAGE}\n")
      message(FATAL_ERROR "${__msg}")
    else()
      message(FATAL_ERROR "${__msg}")
    endif()
  ...
  elseif (FPHSA_NAME_MISMATCHED)
    message(FATAL_ERROR "The package name passed to `find_package_handle_standard_args` (${
_NAME}) does not match the name of the calling package (${CMAKE_FIND_PACKAGE_NAME}). This can lead to problems in calling code that expects `find_package` result variables (e.g., `_FOUND`) to follow a certain pattern.")
  endif()
endmacro()

# This macro is used to print the success message when used in CONFIG_MODE:
macro(FIND_PACKAGE_MESSAGE _NAME _SUCCESS_MSG _DETAILS)
  if(${_NAME}_CONFIG)
    # If the config-file was found, print the success message:
    if(FPHSA_NAME_MISMATCHED)
      message(STATUS "${_SUCCESS_MSG} (package name mismatch: ${CMAKE_FIND_PACKAGE_NAME} vs ${_NAME})")
    else()
      message(STATUS "${_SUCCESS_MSG}")
    endif()
    if(_DETAILS)
      message(STATUS "  Details: ${_DETAILS}")
    endif()
  else()
    # If the config-file was not found, print a warning:
    message(WARNING "${_NAME}Config.cmake or ${_NAME_LOWER}-config.cmake not found, but required by ${_SUCCESS_MSG}")
  endif()
