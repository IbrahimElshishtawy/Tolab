...
List<Middleware<AppState>> createAdminMiddlewares() {
  return [
    ...
    TypedMiddleware<AppState, FetchAdminSubjectsAction>(
      _fetchSubjectsMiddleware,
    ).call,
  ];
}

void _fetchUsersMiddleware(
  Store<AppState> store,
  FetchUsersAction action,
  NextDispatcher next,
) async {
  next(action);
  ...
  try {
    ...
    final users = List.generate(
      10,
      (i) => {'id': i, 'name': 'User ${i + 1}', 'email': 'user$i@uni.edu'},
    );
...
void _fetchSubjectsMiddleware(
  Store<AppState> store,
  FetchAdminSubjectsAction action,
  NextDispatcher next,
) async {
  next(action);
  store.dispatch(AdminLoadingAction(true));
  try {
    ...
    final subjects = List.generate(
      8,
      (i) => {'id': i, 'name': 'Subject ${i + 1}', 'code': 'SUB${100 + i}'},
    );
